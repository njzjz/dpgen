...
def comput_e_shift(poscar, task_name):
    ...
    if len(a_types) > 1:
        if not os.path.isdir('stables'):
            raise RuntimeError(
                'no dir "stable". Stable energy and volume of components should be computed before calculating formation energy of an alloy')
...
def comput_lmp_nev(conf_dir, task_name, write_stable=False):
    ...
    if write_stable:
        ...
        if len(ele_types) > 1:
            raise RuntimeError(
                'stable energy and volume only for one element, current you have %s from POSCAR' % str(ele_types))
    ...
    if os.path.isfile(log_lammps):
        ...
        if write_stable:
            ...
            name_prefix = os.path.join(
                stable_dir, '%s.%s' % (ele_types[0], task_name))
...
def comput_vasp_nev(jdata, conf_dir, write_stable=False):
    ...
    if os.path.isfile(outcar):
        ...
        if write_stable:
            ...
            os.makedirs(stable_dir, exist_ok=True)
            name_prefix = os.path.join(
                stable_dir, '%s.' % (ele_types[0])+vasp_str)
            open(name_prefix + '.e', 'w').write('%.16f\n' % (epa))
            open(name_prefix + '.v', 'w').write('%.16f\n' % (vpa))
        return natoms, epa, vpa
    else:
