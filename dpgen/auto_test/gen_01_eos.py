...
def make_vasp(jdata, conf_dir):
    ...
    if is_alloy:
        fc = vasp.make_vasp_relax_incar(
            ecut, ediff, True,  True, False, npar, kpar, kspacing, kgamma)
    else:
        fc = vasp.make_vasp_relax_incar(
            ecut, ediff, False, True, False, npar, kpar, kspacing, kgamma)
    ...
    for vol in np.arange(vol_start, vol_end, vol_step):
        ...
        os.symlink(os.path.relpath(
            os.path.join(vasp_path, 'POTCAR')), 'POTCAR')
        # gen poscar
...
def make_deepmd_lammps(jdata, conf_dir):
    ...
    for vol in np.arange(vol_start, vol_end, vol_step):
        ...
        fc = lammps.make_lammps_press_relax(
            'conf.lmp', ntypes, scale, lammps.inter_deepmd, deepmd_models_name)
