...
def _make_deepmd_reprod_traj(jdata, conf_dir, supercell, insert_ele, task_name):
    ...
    copy_str = "%sx%sx%s" % (supercell[0], supercell[1], supercell[2])
    struct_widecard = os.path.join(
        vasp_path, 'struct-%s-%s-*' % (insert_ele, copy_str))
    ...
    for vs in vasp_struct:
        ...
        if xdat_nframes > len(energies):
            warnings.warn('nframes %d in xdat is larger than energy %d, use the last %d frames' % (
...
#    print('# generate %s task with conf %s' % (args.TASK, args.CONF))
    if args.TASK == 'vasp':
        ...
        make_deepmd_lammps(jdata, args.CONF, args.COPY,
                           args.ELEMENT, args.TASK)
    elif args.TASK == 'deepmd-reprod':
        make_deepmd_reprod_traj(
            jdata, args.CONF, args.COPY, args.ELEMENT, args.TASK)
    elif args.TASK == 'meam':
        make_meam_lammps(jdata, args.CONF, args.COPY, args.ELEMENT, args.TASK)
    elif args.TASK == 'meam-reprod':
        make_meam_reprod_traj(jdata, args.CONF, args.COPY,
                              args.ELEMENT, args.TASK)
    else:
