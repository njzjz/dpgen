...
def make_vasp(jdata, conf_dir, max_miller=2, relax_box=False, static=False):
    ...
    all_slabs = generate_all_slabs(
        ss, max_miller, min_slab_size, min_vacuum_size)
    # gen incar
    if static:
        ...
        fc = vasp.make_vasp_relax_incar(
            ecut, ediff, True, relax_box, False, npar=npar, kpar=kpar, kspacing=kspacing, kgamma=kgamma)
    ...
    for ii in range(len(all_slabs)):
        ...
        struct_path = os.path.join(
            task_path, 'struct-%03d-%s' % (ii, miller_str))
...
def make_meam_lammps(jdata, conf_dir, max_miller=2, static=False, relax_box=False, task_name='wrong-task'):
    fp_params = jdata['vasp_params']
    kspacing = fp_params['kspacing']
...
def _main():
    ...
    if args.TASK == 'vasp':
        make_vasp(jdata, args.CONF, args.MAX_MILLER,
                  static=False, relax_box=args.relax_box)
    elif args.TASK == 'vasp-static':
        make_vasp(jdata, args.CONF, args.MAX_MILLER,
                  static=True, relax_box=args.relax_box)
    elif args.TASK == 'deepmd':
        make_deepmd_lammps(jdata, args.CONF, args.MAX_MILLER,
                           static=False, relax_box=args.relax_box, task_name=args.TASK)
    elif args.TASK == 'deepmd-static':
        make_deepmd_lammps(jdata, args.CONF, args.MAX_MILLER,
                           static=True, relax_box=args.relax_box, task_name=args.TASK)
    elif args.TASK == 'meam':
        make_meam_lammps(jdata, args.CONF, args.MAX_MILLER, static=False,
                         relax_box=args.relax_box, task_name=args.TASK)
    elif args.TASK == 'meam-static':
        make_meam_lammps(jdata, args.CONF, args.MAX_MILLER, static=True,
                         relax_box=args.relax_box, task_name=args.TASK)
    else:
