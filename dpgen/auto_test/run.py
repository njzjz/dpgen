...
def cmpt_vacancy(task_type, jdata, mdata):
    ...
    # lammps
        ...
        cmpt_03_vacancy.cmpt_deepmd_lammps(
            jdata, conf_dir, supercell, task_type)
    else:
        raise RuntimeError("unknow task ", task_type)
...
def run_interstitial(task_type, jdata, mdata):
    ...
    reprod_opt = jdata['reprod-opt']
    work_path = util.make_work_path(
        jdata, '04.interstitial', reprod_opt, False, False)
    all_task = glob.glob(os.path.join(work_path, 'struct-*'))
...
def gen_surf(task_type, jdata, mdata):
    ...
    # vasp
    if task_type == "vasp":
        gen_05_surf.make_vasp(jdata, conf_dir, max_miller,
                              static=static, relax_box=relax_box)
    # lammps
    elif task_type in lammps_task_type:
        gen_05_surf.make_lammps(jdata, conf_dir, max_miller,
                                static=static, relax_box=relax_box, task_type=task_type)
    else:
        raise RuntimeError("unknow task ", task_type)
    os.chdir(cwd)
...
def cmpt_surf(task_type, jdata, mdata):
    ...
    # lammps
        ...
        if static_opt:
            ...
            task_name = task_type
        cmpt_05_surf.cmpt_deepmd_lammps(
            jdata, conf_dir, task_name, static=static_opt)
    else:
        raise RuntimeError("unknow task ", task_type)
    os.chdir(cwd)
...
def run_task(json_file, machine_file):
    ...
    ii = jdata['task_type']
    jj = jdata['task']
    task_list = ['equi', 'eos', 'elastic', 'vacancy',
                 'interstitial', 'surf', 'phonon', 'all']
