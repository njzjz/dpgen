...
class AWS(Batch):
        ...
        if datetime.now().timestamp() > cls._query_next_allow_time:
            ...
            for status in ['SUBMITTED', 'PENDING', 'RUNNABLE', 'STARTING', 'RUNNING', 'SUCCEEDED', 'FAILED']:
                status_response = cls.batch_client.list_jobs(
                    jobQueue=cls._jobQueue, jobStatus=status, maxResults=cls._query_max_results)
                status_list = status_response.get('jobSummaryList', [])
                for job_dict in status_list:
                    cls._job_id_map_status.update(
                        {job_dict['jobId']: cls.map_aws_status_to_dpgen_status(job_dict['status'])})
    ...
    def job_id(self):
        try:
            ...
            if self.context.check_file_exists(self.job_id_name):
                ...
                response_list = self.__class__.batch_client.describe_jobs(
            ...
            dlog.debug("50000, self._job_id:%s,_Queue:%s,_map:%s," % (
                self._job_id, self.__class__._jobQueue, self.__class__._job_id_map_status))
            return ""
        return self._job_id
    ...
    def job_id(self, values):
        ...
        self.__class__._job_id_map_status[self._job_id] = self.map_aws_status_to_dpgen_status(
            response.get('status', 'SUBMITTED'))
        self.context.write_file(self.job_id_name, self._job_id)
        dlog.debug("15000, _job_id:%s, _job_name:%s, _map:%s, _Queue:%s" % (
            self._job_id, self._job_name, self.__class__._job_id_map_status, self.__class__._jobQueue))
    ...
    def do_submit(self,
        ...
        dlog.debug('2200, self.context.remote_root: %s , self.context.local_root: %s' % (
            self.context.remote_root, self.context.local_root))
        # concreate_command =
        script_str = self.sub_script(
            job_dirs, cmd, args=args, res=res, outlog=outlog, errlog=errlog)
        ...
        """
        jobName = os.path.join(self.context.remote_root, job_dirs.pop())[
            1:].replace('/', '-').replace('.', '_')
