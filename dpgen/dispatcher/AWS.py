...
class AWS(Batch):
    ...
    def AWS_check_status(cls, job_id=""):
        ...
        if datetime.now().timestamp() > cls._query_next_allow_time:
            ...
            cls._query_next_allow_time = (
                datetime.now().timestamp() + cls._query_time_interval
            )
            for status in [
                "SUBMITTED",
                "PENDING",
                ...
                for job_dict in status_list:
                    cls._job_id_map_status.update(
                        {
                            job_dict["jobId"]: cls.map_aws_status_to_dpgen_status(
                                job_dict["status"]
                            )
                        }
                    )
        ...
        dlog.debug(
            "62000:job_id:%s, _query: %s, _map: %s"
        ...
        try:
            ...
            if self.context.check_file_exists(self.job_id_name):
                ...
                try:
                    ...
                    return self._job_id
            dlog.debug(
                "50000, self._job_id:%s,_Queue:%s,_map:%s,"
                % (
                    self._job_id,
    ...
    def job_id(self, values):
        ...
        self.__class__._job_id_map_status[
            self._job_id
        ] = self.map_aws_status_to_dpgen_status(response.get("status", "SUBMITTED"))
    ...
    def do_submit(self, job_dirs, cmd, args=None, res=None, outlog="log", errlog="err"):

        res = self.default_resources(res)
        dlog.debug(
            "2000, params=(%s, %s, %s, %s, %s, %s, )"
        ...
        dlog.debug(
            "2300, script_str: %s, self.sub_script_name: %s"
        ...
        """
        jobName = (
            os.path.join(self.context.remote_root, job_dirs.pop())[1:]
            .replace("/", "-")
            .replace(".", "_")
        )
