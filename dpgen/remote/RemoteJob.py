...
class awsMachineJob(object):
    ...
    def upload(self,
        ...
        for ii in local_up_files:
            print('self.local_root=', self.local_root, 'remote_root=',
                  self.remote_root, 'job_dir=', job_dir, 'ii=', ii)
            if os.path.isfile(os.path.join(job_dir, ii)):
                ...
                shutil.copyfile(os.path.join(job_dir, ii),
                                os.path.join(self.remote_root, job_dir, ii))
            elif os.path.isdir(os.path.join(job_dir, ii)):
                shutil.copytree(os.path.join(job_dir, ii),
                                os.path.join(self.remote_root, job_dir, ii))
            else:
                print('unknownfile', 'local_root=', self.local_root,
                      'job_dir=', job_dir, 'filename=', ii)
        os.chdir(cwd)
         ...
         #   print('self.local_root=',self.local_root,'remote_root=',self.remote_root,'job_dir=',job_dir,'ii=',ii)
            ...
            while not file_succ_copy_flag:
                if os.path.isfile(os.path.join(self.remote_root, job_dir, ii)):
                    shutil.copyfile(os.path.join(self.remote_root, job_dir, ii), os.path.join(
                        self.local_root, job_dir, ii))
                    ...
                    try:
                        ...
                        print('dir is not empty   ' +
                              str(os.path.join(self.local_root, job_dir, ii)))
                    else:
                        shutil.copytree(os.path.join(self.remote_root, job_dir, ii), os.path.join(
                            self.local_root, job_dir, ii))
                        file_succ_copy_flag = True
                else:
                    print('unknownfile,maybe need for waiting for a while', 'local_root=',
                          self.local_root, 'job_dir=', job_dir, 'filename=', ii)
                    time.sleep(5)
...
class SSHSession (object):
    def __init__(self, jdata):
        ...
        self.ssh = self._setup_ssh(self.remote_host, self.remote_port,
                                   username=self.remote_uname, password=self.remote_password)
...
class RemoteJob (object):
    def __init__(self,
        ...
        self.remote_root = os.path.join(
            ssh_session.get_session_root(), self.job_uuid)
    ...
    def block_checkcall(self,
        ...
        if exit_status != 0:
            raise RuntimeError(
                "Get error code %d in calling %s through ssh with job: %s ", (exit_status, cmd, self.job_uuid))
...
class SlurmJob (RemoteJob):
    ...
    def check_status(self):
        ...
        if job_id == "":
            raise RuntimeError("job %s has not been submitted" %
                               self.remote_root)
...
class PBSJob (RemoteJob):
    ...
    def check_status(self):
        ...
        if job_id == "":
            raise RuntimeError(
                "job %s is has not been submitted" % self.remote_root)
        ret, stdin, stdout, stderr\
...
class LSFJob (RemoteJob):
    ...
    def _submit(self,
        ...
        script_name = self._make_script(job_dirs, cmd, args, res=resources)
        stdin, stdout, stderr = self.block_checkcall(
            ('cd %s; bsub < %s' % (self.remote_root, script_name)))
        subret = (stdout.readlines())
