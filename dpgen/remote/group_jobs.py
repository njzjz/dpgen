...
def aws_submit_jobs(machine,
    ...
    task_chunks = (str(task_chunks).translate(
        (str.maketrans('', '', ' \'"[]'))).split(','))
    ...
    while not len(status_list) >= group_size:
        containerInstanceArns = ecs.list_container_instances(
            cluster="tensorflow")
        if containerInstanceArns['containerInstanceArns']:
            containerInstances = ecs.describe_container_instances(cluster="tensorflow",
                                                                  containerInstances=containerInstanceArns['containerInstanceArns'])['containerInstances']
            status_list = [container['status']
                           for container in containerInstances]
    ...
    for ijob in range(njob):  # uplaod && submit job
        ...
        while running_job_num >= group_size:
            taskstatus = [task['lastStatus'] for task in ecs.describe_tasks(
                cluster='tensorflow', tasks=taskARNs)['tasks']]
            running_job_num = len(
                list(filter(lambda str: (str == 'PENDING' or str == 'RUNNING'), taskstatus)))
            print('waiting for running job finished, taskstatus=',
                  taskstatus, 'running_job_num=', running_job_num)
            time.sleep(10)
        ...
        while not taskres['tasks'][0]:
            print('task submit failed,taskres=', taskres,
                  'trying to re-submit'+str(chunk),)
            time.sleep(10)
            taskres = ecs.run_task(cluster='tensorflow',
                                   taskDefinition=task_definition, overrides=command_override)
        ...
        running_job_num = len(
            list(filter(lambda str: (str == 'PENDING' or str == 'RUNNING'), taskstatus)))
        print('have submitted %s/%s,taskstatus=' % (work_path, chunk),
              taskstatus, 'running_job_num=', running_job_num)
    task_fin_flag = False
    while not task_fin_flag:
        ...
        if task_fin_flag:
            print(
                'task finished,next step:copy files to local && taskstatus=', taskstatus)
        else:
...
def _ucloud_remove_machine(machine, UHostId):
    ...
    ucloud_stop_param['Signature'] = _verfy_ac(
        machine['Private'], ucloud_stop_param)
    ...
    while not terminate_fin:
        ...
        ucloud_delete_param['Signature'] = _verfy_ac(
            machine['Private'], ucloud_delete_param)
...
def ucloud_submit_jobs(machine,
    ...
    if continue_status == False:
        ...
        ucloud_start_param['Signature'] = _verfy_ac(
            machine['Private'], ucloud_start_param)
    ...
    while not all(machine_fin):
        ...
        ucloud_check_param1['Signature'] = _verfy_ac(
            machine['Private'], ucloud_check_param1)
        ...
        for idx, rjob in enumerate(job_list):
            if not job_fin[idx]:
                ...
                if status == JobStatus.terminated:
                    raise RuntimeError(
                        "find unsuccessfully terminated job on machine" % ucloud_machines[idx])
            ...
            if not job_fin[idx]:
                try:
                    ...
                    for _idx, _rjob in enumerate(job_list):
                        job_list[_idx] = SlurmJob(
                            ssh_sess, work_path, _rjob.job_uuid)
                ...
                if status == JobStatus.terminated:
                    ...
                    rjob.submit(task_chunks[idx], command,
                                resources=resources, restart=True)
                    if lcount[idx] > 3:
                        dlog.info(
                            'program has to exit due to too many errors ! ')
                        raise RuntimeError(
                            "find unsuccessfully terminated job in %s" % rjob.get_job_root())
                elif status == JobStatus.finished:
                    rjob.download(task_chunks[idx], backward_task_files)
                    rjob.clean()
                    job_fin[idx] = True
