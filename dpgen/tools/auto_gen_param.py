...
class System(object):
    ...
    def add_sub_system(self, idx2, files_list):
        ...
        self.sub_system_list.append(
            (idx1, self.index_system, idx2, files_list))
        self.current_idx2 += 1
...
class Iteration(object):
    ...
    def gen_sub_iter(self, system_list):
        ...
        for idx2 in range(self.sub_iteration_num):
            ...
            iter_dict['sys_idx'] = [ii[0]
                                    for ii in system_list if ii[2] == idx2]
    ...
    for system_prefix, data_list in system_dict.items():
        if map_iterator is None:
            ...
            origin_one, new_map_iterator = tee(
                map_iterator)  # pylint: disable=unused-variable
        ...
        for idx2 in range(sub_iteration_num):
            files_list = [os.path.join(system_prefix, jj)
                          for jj in next(new_map_iterator)]
...
def scan_files(scan_dir="./", file_name="POSCAR", min_allow_files_num=20):
    ...
    for ii in os.walk(scan_dir):
        if file_name in ii[2]:
            ...
            system_dict[system_prefix].append(
                os.path.join(system_suffix, file_name))
...
def get_model_devi_jobs(melt_point,
    ...
    if len(nsteps_list) != sub_iteration_num:
        raise RuntimeError(
            f"{nsteps_list}, {sub_iteration_num};length do not match!")
    model_devi_jobs = []
...
def get_basic_param_json(melt_point,
    ...
    init_data_sys = get_init_data_sys(
        scan_dir=scan_dir, init_file_name=init_file_name)
