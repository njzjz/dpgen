...
def collect_data(target_folder, param_file, output, verbose=True):
    ...
    command_shuffle_raw = os.path.join(
        tool_path, 'tools.raw', 'shuffle_raw.py')
    ...
    for idx, ii in enumerate(coll_sys):
        ...
        # cat data.configs
        data_configs = glob.glob(os.path.join(
            'iter.[0-9]*[0-9].data.[0-9]*[0-9]', 'orig', 'data.configs'))
        data_configs.sort()
        ...
        os.chdir('..')
        # shuffle raw
        sp.check_call(command_shuffle_raw + ' orig ' +
                      ' . > /dev/null', shell=True)
...
def _main():
    parser = argparse.ArgumentParser(
        description='Collect data from DP-GEN iterations')
