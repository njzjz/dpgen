...
def create_init_tasks(target_folder, param_file, output, fp_json, verbose=True):
    ...
    tool_path = os.path.join(os.path.dirname(
    ...
    for idx, ii in enumerate(init_data_sys):
        sys = dpdata.LabeledSystem(os.path.join(
            init_data_prefix, ii), fmt='deepmd/npy', type_map=type_map)
        ...
        for ff in range(nframes):
            ...
            if fp_style == 'vasp':
                ...
                os.symlink(os.path.relpath(
                    os.path.join(output, 'INCAR')), 'INCAR')
            elif fp_style == 'pwscf':
                try:
                    ...
                    user_input = False
                make_pwscf('.', fp_params, mass_map,
                           fp_pp_files, fp_pp_files, user_input)
...
def create_tasks(target_folder, param_file, output, fp_json, verbose=True, numb_iter=-1):
    ...
    for ii in iters[:numb_iter]:
        iter_tasks = glob.glob(os.path.join(
            ii, '02.fp', 'task.[0-9]*[0-9].[0-9]*[0-9]'))
        iter_tasks.sort()
        if verbose:
        ...
        for jj in iter_tasks:
            ...
            task_record = linked_keys[-5] + '.' + \
                linked_keys[-3] + '.' + linked_keys[-1].split('.')[0]
            task_record_keys = task_record.split('.')
            ens, temp, pres = get_lmp_info(os.path.join(
                ii, '01.model_devi', linked_keys[-3], 'input.lammps'))
    ...
    for si in range(numb_sys):
        ...
        for tt, rr in zip(sys_tasks[si], sys_tasks_record[si]):
            ...
            source_path = os.path.join(
                ('iter.%s/02.fp' % rr.split()[1]), rr.split()[9])
...
def _main():
    ...
    create_tasks(args.JOB_DIR, args.parameter, args.OUTPUT,
                 args.PARAM, numb_iter=args.numb_iter, verbose=args.verbose)
    create_init_tasks(args.JOB_DIR, args.parameter,
                      args.OUTPUT, args.PARAM, verbose=args.verbose)
