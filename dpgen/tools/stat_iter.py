        ...
        if line:
            ...
            try:
                _sys = dpdata.LabeledSystem(
                    path_doc, type_map=jdata['type_map'])
            except:
                try:
            ...
            if len(_sys) == 1:
                ...
                if verbose:
                    print(
                        'OUTCAR not label by dpdata, not convergence or unfinshed', path_doc)
    ...
    for pk_id in {**iter_dict}:
        if iter_dict[pk_id]['OUTCAR_total_count']:
            iter_dict[pk_id]['reff'] = round(
                iter_dict[pk_id]['OUTCAR_not_convergence']/iter_dict[pk_id]['OUTCAR_total_count'], 5)
