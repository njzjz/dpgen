...
class Test(unittest.TestCase):
    ...
    def testVaspInput(self):
        for f in self.init_path:
            ...
            self.assertEqual(str(vi['POTCAR']), str(
                self.ref_init_input['POTCAR']))
    ...
    def testParsingVasp(self):
        parsing_vasp(self.cwd, self.config_info_dict, self.skip_init,
                     self.output, id_prefix=dpgen.SHORT_CMD)
        # try:
        #   Potcar(['Al'])
