...
def _check_poscars(testCase, idx, fp_task_max, type_map):
    ...
    for sidx, ii in zip(sys_idx, candi_files):
        ...
        for tt, ff in zip(md_task, f_idx):
            ...
            cc += 1
            sys0 = dpdata.System(
                traj_file, fmt='lammps/dump', type_map=type_map)
    ...
    for ii in tasks:
        ...
        if isinstance(gamma, bool):
            ...
            if gamma[0].upper() == "T":
                ...
                gamma = False
        ret = make_kspacing_kpoints(os.path.join(
            os.path.join(ii, 'POSCAR')), kspacing, gamma)
...
def _check_potcar(testCase, idx, fp_pp_path, fp_pp_files):
    ...
    for ii in range(nfile):
        testCase.assertTrue(os.path.isfile(
            os.path.join(fp_pp_path, fp_pp_files[ii])))
    tasks = glob.glob(os.path.join(fp_path, 'task.*'))
    for ii in tasks:
...
def _check_incar_ele_temp(testCase, idx, ele_temp):
    ...
    for ii in tasks:
        ...
        with open('INCAR') as fp:
            ...
            incar1 = Incar.from_string(vasp_incar_ele_temp_ref % (
...
class TestMakeFPPwscf(unittest.TestCase):
    def test_make_fp_pwscf(self):
        ...
        make_fp(0, jdata, {})
        _check_sel(self, 0, jdata['fp_task_max'],
                   jdata['model_devi_f_trust_lo'], jdata['model_devi_f_trust_hi'])
