...
class TestRunReport(unittest.TestCase):
    def test_stat_sys(self):
        ...
        sys, sys_count, sys_all = stat_sys(os.path.join(
            test_dir, folder), verbose=False, mute=True)
        with open(os.path.join(test_dir, folder, 'param.json')) as fp:
            jdata = json.load(fp)
        self.assertEqual(sys, jdata['sys_configs'])
        self.assertEqual(
            sys_count, [jdata['fp_task_max'], jdata['fp_task_max']])
        ref_all = [[['npt', 50.0, 1.0, 4], ['npt', 50.0, 2.0, 1], ['npt', 100.0, 1.0, 2], [
            'npt', 100.0, 2.0, 1]], [['npt', 50.0, 1.0, 2], ['npt', 50.0, 2.0, 4], ['npt', 100.0, 1.0, 2]]]
